...
async function ensureColumns(env, table, columns) {
  const info = await dbAll(env, `PRAGMA table_info(${table})`);
  const existing = new Set(info.map((col) => col.name));
  for (const column of columns) {
    if (!existing.has(column.name)) {
      await dbRun(env, `ALTER TABLE ${table} ADD COLUMN ${column.name} ${column.definition}`);
    }
  }
}
...
export async function initDb(env) {
  ...
  initPromise = (async () => {
    ...
    await ensureColumns(env, 'products', [
      ...
      { name: 'rating', definition: 'REAL' },
      { name: 'review_count', definition: 'INTEGER' },
      { name: 'subcategory', definition: 'TEXT' },
    ]);

    const count = await dbGet(env, 'SELECT COUNT(*) as count FROM products');
    ...
    const orderCount = await dbGet(env, 'SELECT COUNT(*) as count FROM orders');
